      ...
      <nav className="backdrop-blur-[8px] bg-[rgba(10,10,10,0.95)] h-[66px] shrink-0 sticky top-0 w-full z-50 border-b border-[#2a2a2a]">
        <div className="flex items-center justify-between h-full px-[40px]">
          ...
          <div className="flex items-center gap-4">
            <span className="text-[#888] text-[13px]">{user?.full_name || 'Student'}</span>
            <button
      ...
      <div className="flex min-h-[calc(100vh-66px)]">
        ...
        <div className="fixed left-0 top-[66px] w-[240px] h-[calc(100vh-66px)] bg-[#0f0f0f] border-r border-[#2a2a2a] overflow-y-auto">
          <div className="flex flex-col py-5 px-0">
            ...
            <Link to="/dashboard" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>🏠</span><span className="text-[14px]">Home</span>
            </Link>
            <Link to="/courses" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>📚</span><span className="text-[14px]">My Courses</span>
            </Link>
            <Link to="/assignments" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>📝</span><span className="text-[14px]">Assignments</span>
            </Link>
            <Link to="/messages" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center bg-[rgba(191,255,0,0.1)] border-l-2 border-[#bfff00] no-underline">
              <span className="text-[#bfff00]">💬</span>
              <span className="text-[#bfff00] text-[14px]">Messages</span>
            </Link>
            <Link to="/live-sessions" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>🎥</span><span className="text-[14px]">Live Classes</span>
            </Link>
            <div className="text-[#555] text-[9px] uppercase tracking-[1.35px] px-6 py-3 mt-4">Progress</div>
            <Link to="/progress" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>📊</span><span className="text-[14px]">Progress</span>
            </Link>
            <Link to="/achievements" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center text-[#888] hover:text-white no-underline">
              <span>🏆</span><span className="text-[14px]">Achievements</span>
            </Link>
          </div>
        </div>
        ...
        <div className="ml-[240px] flex-1 p-9">
          <div className="mb-6">
            <h1 className="text-[28px] text-white font-bold">Messages</h1>
            <p className="text-[#888] text-[14px] mt-1">Chat with your instructors</p>
          </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {loading ? (
                  ...
                  <div className="text-center py-10 px-4">
                    ...
                    <p className="text-[#555] text-[11px] mt-1">Your instructor will reach out when needed</p>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {conv.instructor_name.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-white text-[13px] font-medium truncate">{conv.instructor_name}</span>
                          <span className="text-[#555] text-[10px] flex-shrink-0 ml-2">{formatTime(conv.last_message_at)}</span>
                        </div>
                        <div className="flex items-center justify-between mt-0.5">
                          <span className="text-[#888] text-[11px] truncate">{conv.last_message_preview || 'No messages yet'}</span>
                          {conv.unread_count > 0 && (
              ...
              {selectedConversation ? (
                  ...
                  <div className="p-4 border-b border-[#2a2a2a]">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ background: 'linear-gradient(135deg, #bfff00, #8fef00)' }}
                      >
                        {selectedConversation.instructor_name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="text-white font-semibold text-[14px]">{selectedConversation.instructor_name}</h3>
                ...
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-[#555] text-[12px] mt-1">Messages from your instructors appear here</p>
                  </div>
                </div>
              )}
            </div>
