      ...
      <div className="flex min-h-[calc(100vh-66px)]">
        ...
        <div className="fixed left-0 top-[66px] w-[260px] h-[calc(100vh-66px)] bg-[#0f0f0f] border-r border-[#2a2a2a] overflow-y-auto">
          <div className="flex flex-col py-5 px-0">
            <div className="text-[#555] text-[9px] uppercase tracking-[1.35px] px-6 py-3">Billing</div>
            
            <Link to="/admin/revenue" className="w-full py-3 pl-6 pr-4 flex gap-3 items-center bg-[rgba(191,255,0,0.1)] border-l-2 border-[#bfff00]">
              <span className="text-[#bfff00]">💰</span>
              <span className="text-[#bfff00] text-[14px]">Revenue</span>
            </Link>
        ...
        <div className="ml-[260px] flex-1 p-9">
          <div className="mb-8">
            <h1 className="text-[32px] text-white font-bold">
              <span className="text-[#bfff00]">Revenue</span> & Billing
            </h1>
          ...
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
              <div className="text-[#888] text-[12px] uppercase">Monthly Revenue</div>
              <div className="text-[32px] text-white font-bold mt-2">${mrr.toFixed(2)}</div>
              <div className="text-[#00ff7f] text-[12px] mt-2">MRR</div>
            </div>
            
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
              <div className="text-[#888] text-[12px] uppercase">Total Revenue</div>
              <div className="text-[32px] text-white font-bold mt-2">${totalRevenue.toFixed(2)}</div>
              <div className="text-[#888] text-[12px] mt-2">All time</div>
            </div>
            
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
              <div className="text-[#888] text-[12px] uppercase">Pending Payouts</div>
              <div className="text-[32px] text-white font-bold mt-2">${pendingPayouts.toFixed(2)}</div>
              <div className="text-orange-400 text-[12px] mt-2">{payouts.filter(p => p.status === 'pending').length} pending</div>
            </div>
            
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
              <div className="text-[#888] text-[12px] uppercase">Refunds</div>
              <div className="text-[32px] text-white font-bold mt-2">${totalRefunds.toFixed(2)}</div>
              <div className="text-red-400 text-[12px] mt-2">Total issued</div>
            </div>
          </div>
                    ...
                    {transactions.map((tx) => (
                      <tr key={tx.id} className="border-b border-[#2a2a2a]">
                        <td className="py-4 px-6">
                          <div className="text-white">{tx.user_name}</div>
                          <div className="text-[#555] text-sm">{tx.user_email}</div>
                        </td>
                        <td className="py-4 px-6">
                          <span className={`px-2 py-1 rounded-[4px] text-xs capitalize ${
                            tx.type === 'purchase' ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' :
                            tx.type === 'refund' ? 'bg-[rgba(255,0,0,0.1)] text-red-500' :
                            'bg-[rgba(255,165,0,0.1)] text-orange-400'
                          }`}>
                            {tx.type}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-white">{tx.description}</td>
                        <td className={`py-4 px-6 font-bold ${tx.amount >= 0 ? 'text-[#00ff7f]' : 'text-red-500'}`}>
                          {tx.amount >= 0 ? '+' : ''}${tx.amount.toFixed(2)}
                        </td>
                        <td className="py-4 px-6">
                          <span className={`px-2 py-1 rounded-[4px] text-xs ${
                            tx.status === 'completed' ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' :
                            tx.status === 'pending' ? 'bg-[rgba(255,165,0,0.1)] text-orange-400' :
          ...
          {activeTab === 'subscriptions' && (
            <div className="grid grid-cols-3 gap-4">
              {plans.map((plan) => (
                <div key={plan.id} className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-bold text-lg">{plan.name}</h3>
                    <span className={`px-2 py-1 rounded-[4px] text-xs ${plan.is_active ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' : 'bg-[rgba(255,0,0,0.1)] text-red-500'}`}>
                      {plan.is_active ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  <div className="mb-4">
                    <span className="text-[32px] text-white font-bold">${plan.price}</span>
                    <span className="text-[#888]">/{plan.interval}</span>
                  </div>
                  <div className="text-[#888] text-sm mb-4">{plan.subscriber_count} subscribers</div>
                  <ul className="space-y-2">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="text-white text-sm flex items-center gap-2">
                        <span className="text-[#bfff00]">✓</span> {feature}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          {activeTab === 'promos' && (
            <div className="space-y-4">
              {promos.map((promo) => (
                <div key={promo.id} className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="bg-[#1f1f1f] px-4 py-2 rounded-[8px]">
                      <span className="text-[#bfff00] font-bold">{promo.code}</span>
                    </div>
                    <div>
                  ...
                  <span className={`px-3 py-1 rounded-[99px] text-xs ${promo.is_active ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' : 'bg-[rgba(255,0,0,0.1)] text-red-500'}`}>
          ...
          {activeTab === 'payouts' && (
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {payouts.map((payout) => (
                    <tr key={payout.id} className="border-b border-[#2a2a2a]">
                      <td className="py-4 px-6">
                        <div className="text-white">{payout.instructor_name}</div>
                        <div className="text-[#555] text-sm">{payout.instructor_email}</div>
                      </td>
                      <td className="py-4 px-6 text-white font-bold">${payout.amount.toFixed(2)}</td>
                      <td className="py-4 px-6">
                        <span className={`px-2 py-1 rounded-[4px] text-xs capitalize ${
                          payout.status === 'completed' ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' :
                          payout.status === 'pending' ? 'bg-[rgba(255,165,0,0.1)] text-orange-400' :
                          payout.status === 'processing' ? 'bg-[rgba(0,191,255,0.1)] text-[#00bfff]' :
                          'bg-[rgba(255,0,0,0.1)] text-red-500'
                        }`}>
                          {payout.status}
                        </span>
                      ...
                      <td className="py-4 px-6">
                        {payout.status === 'pending' && (
                          <button className="bg-[#bfff00] text-black px-4 py-1 rounded-[4px] text-sm font-medium">
                            Process
                          </button>
                        )}
                      </td>
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-[#151515] border border-[#2a2a2a] rounded-[14px] p-6">
                <h3 className="text-white font-bold mb-4">Top Plans</h3>
                <div className="space-y-3">
                  {plans.map((plan) => (
                    <div key={plan.id} className="flex items-center justify-between">
                      <span className="text-white">{plan.name}</span>
                      <span className="text-[#bfff00] font-bold">{plan.subscriber_count}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {transactions.slice(0, 5).map((tx) => (
                    <div key={tx.id} className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-[#555] text-sm ml-2">{new Date(tx.created_at).toLocaleDateString()}</span>
                      </div>
                      <span className={tx.amount >= 0 ? 'text-[#00ff7f]' : 'text-red-500'}>
