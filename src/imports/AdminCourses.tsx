      ...
      <div className="flex min-h-[calc(100vh-66px)]">
        ...
        <div className="fixed left-0 top-[66px] w-[240px] h-[calc(100vh-66px)] bg-[#0f0f0f] border-r border-[#2a2a2a] overflow-y-auto">
          <div className="flex flex-col py-5 px-0">
            ...
            <Link to="/admin/courses" className="w-full bg-[rgba(191,255,0,0.1)] border-l-2 border-[#bfff00] py-3 pl-6 pr-4 flex gap-3 items-center">
              <span className="text-[#bfff00]">📚</span>
              <span className="text-[#bfff00] text-[14px]">Courses</span>
            </Link>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-[32px] text-white font-bold">
                Course <span className="text-[#bfff00]">Management</span>
              </h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#2a2a2a]">
                  ...
                  <th className="text-left text-[#888] text-[12px] uppercase tracking-[1px] px-6 py-4">Instructor</th>
                  <th className="text-left text-[#888] text-[12px] uppercase tracking-[1px] px-6 py-4">Level</th>
                  <th className="text-left text-[#888] text-[12px] uppercase tracking-[1px] px-6 py-4">Status</th>
                  <th className="text-left text-[#888] text-[12px] uppercase tracking-[1px] px-6 py-4">Enrollments</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="text-center text-[#888] py-8">No courses found</td>
                  </tr>
                ) : (
                  courses.map((course) => (
                    <tr key={course.id} className="border-b border-[#2a2a2a] hover:bg-[#1a1a1a]">
                      <td className="px-6 py-4">
                        <div className="text-white font-medium">{course.title}</div>
                        <div className="text-[#555] text-[12px]">{course.goal}</div>
                      </td>
                      <td className="px-6 py-4 text-[#888]">{course.instructor.name}</td>
                      <td className="px-6 py-4">
                        <span className="bg-[#1f1f1f] text-[#888] px-3 py-1 rounded-[99px] text-[12px]">
                          {course.level}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-[99px] text-[12px] ${
                          course.approval_status === 'approved' ? 'bg-[rgba(0,255,127,0.1)] text-[#00ff7f]' :
                          course.approval_status === 'pending' ? 'bg-[rgba(255,184,0,0.1)] text-[#ffb800]' :
                          'bg-[rgba(255,68,68,0.1)] text-[#f44]'
                        }`}>
                          {course.approval_status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-[#888]">{course.total_enrollments}</td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          {course.approval_status === 'pending' && (
                            <>
                              <button 
                                ...
                                className="bg-[#00ff7f] text-black px-3 py-1 rounded text-[12px] font-medium"
                              >
                                Approve
                              </button>
                              <button 
                                onClick={() => handleReject(course.id)}
                                className="bg-[#f44] text-white px-3 py-1 rounded text-[12px] font-medium"
                              >
                                Reject
                              </button>
                            </>
                          ...
                          {course.approval_status === 'approved' && !course.is_published && (
                            <button 
                              ...
                              className="bg-[#bfff00] text-black px-3 py-1 rounded text-[12px] font-medium"
                            >
                              Publish
                            </button>
                          )}
                          {course.is_published && (
                            <button 
                              ...
                              className="bg-[#ff6b6b] text-white px-3 py-1 rounded text-[12px] font-medium"
                            >
                              Unpublish
                            </button>
                          )}
