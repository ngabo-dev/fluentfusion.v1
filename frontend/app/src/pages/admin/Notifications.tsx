...
export default function Notifications() {
  ...
  const load = () => api.get('/api/admin/notifications').then(r => setNotifs(Array.isArray(r.data) ? r.data : []))
  ...
  const targetLabel = (t: string) => t === 'all' ? 'All Users' : t === 'students' ? 'Students' : t === 'instructors' ? 'Instructors' : t

  return (
    <div className="pg">
      <div className="ph"><div><h1>Notifications</h1><p>Send and manage platform announcements</p></div></div>
          ...
          <tbody>
            {notifs.map(n => (
              <tr key={n.id}>
                ...
                <td style={{ fontSize: 11, color: 'var(--mu)', maxWidth: 260 }}>{n.message?.slice(0, 80)}{n.message?.length > 80 ? '…' : ''}</td>
