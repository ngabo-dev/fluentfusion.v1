...
export default function Login() {
  ...
  const [email, setEmail] = useState('a.ndiaye@ff.com')
  const [pw, setPw] = useState('instructor123')
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ width: 360, background: 'var(--card)', border: '1px solid var(--bdr)', borderRadius: 'var(--rl)', padding: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 28 }}>
          ...
          <span style={{ marginLeft: 'auto', fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--in)', background: 'rgba(0,207,255,.1)', padding: '2px 7px', borderRadius: 4 }}>INSTRUCTOR</span>
        </div>
        ...
        <div style={{ marginTop: 16, fontSize: 10, color: 'var(--mu)', fontFamily: 'JetBrains Mono' }}>
          Demo: a.ndiaye@ff.com / instructor123
        </div>
      </div>
    </div>
  )
