"""
Standalone PULSE model training script.
Reads OULAD data from ../archive/, trains GradientBoostingClassifier,
saves artifacts to pulse_artifacts/.
Run from the PULSE/ directory: python3 train_pulse.py
"""
import numpy as np
import pandas as pd
...
vle    = pd.concat([pd.read_csv(ARCHIVE / f'studentVle_{i}.csv') for i in range(8)], ignore_index=True)
print(f"  info={info.shape}, assess={assess.shape}, reg={reg.shape}, vle={vle.shape}")

# ── 2. Deduplicate info — keep the row with the best final_result per student ─
...
model = GradientBoostingClassifier(n_estimators=300, learning_rate=0.08, max_depth=5, random_state=RANDOM_STATE)
...
print("\n✅  Training complete — model is production-ready.")
